<Meta>
  ...
  <description>
    World ID 4.0 introduces a major protocol upgrade with World ID Accounts,
    multi-key support, and enhanced privacy guarantees. This foundational change
    enables further decentralization, key rotation, recovery mechanisms, and
    protocol-level protection for nullifier anonymity.
  </description>
  <date>2025-12-10</date>
  <category>Developers</category>
  <hero-image src="./World_ID_Registry.png" />
  <social-image src="./World_ID_Registry.png" />
</Meta>

World ID is a privacy-preserving proof of human protocol. Its goal is to enable [World's mission](https://whitepaper.world.org/#introducing-world-network) to build the real human network to accelerate every human in the age of AI. With more than 17 million unique humans already verified, we're proposing a new major version of the Protocol: **World ID 4.0**. This upgrade is designed to unlock the next level of global scale while enhancing the privacy and security promises core to the project.

## Why a new Protocol version?

World ID 3.0 is already at the frontier of privacy, but solely utilizing zero-knowledge proofs (ZKPs) alone is no longer sufficient to meet the growing privacy and security challenges of next-generation digital systems ([Vitalik Buterin, Does digital ID have risks even if it's ZK-wrapped?](https://vitalik.eth.limo/general/2025/06/28/zkid.html)). One challenge to call out is that in the current protocol if the user's secret gets compromised, all past activity could be linked. A foundational change to the Protocol is required to improve security, privacy, and resilience, which in turn enables wider adoption. The **introduction of World ID Accounts** with multi-key support provides the following benefits:
...
> Definition: An *Authenticator* is a software or hardware agent (e.g., app, device, web client, or service) that controls a set of authorized keypairs for a World ID Account and is functionally capable of interacting with the Protocol, and is therefore permitted to act on that account's behalf.

For example, an Authenticator can live in a mobile wallet or a web application. The applications may or may not be dedicated to identity (for instance a social media application may incorporate a World ID Authenticator without the need for a separate app).
...
- **Authenticator plurality.** Beyond increasing resilience of the Protocol, entirely new classes of Authenticators is now possible. For example, Authenticators that rely on secure hardware modules, or Authenticators which use more widespread identification mechanisms.
- **Granular permissions (Future).** In the future, granular controls and permissions for different Authenticators.

This new overhaul also brings increased privacy benefits. Chief among them is the protocol-level protection for nullifier anonymity. This means that with World ID 4.0, new protections are introduced so nullifiers can only ever be used once, preventing nullifiers from becoming a pseudonymous identifier.

## Introducing World ID Accounts

Analogous to how the Ethereum ecosystem introduced [Account Abstraction](https://ethereum.org/roadmap/account-abstraction/), we propose a similar instance of accounts in the World ID Protocol. To put another way, a World ID is no longer a single secret but an abstract record on a public on-chain registry. Here, multiple Authenticators can be authorized to act on its behalf. This registry is called the `WorldIDRegistry`.

![Visual representation of the World ID Registry.](./World_ID_Registry.png)

Visual representation of the World ID Registry.

### Nullifiers

Achieving this abstraction while maintaining the high standard for user privacy is not simple. The World ID Protocol relies on something called *Nullifiers* (previously also called nullifier hashes). A nullifier is a deterministic, unguessable, and uniformly distributed number. Whenever a user presents a Proof in the World ID Protocol, they generate a nullifier for the specific provided context. This number is what provides uniqueness guarantees to a Relying Party (RP). For example, if an RP wants to ensure that a single person is only voting once on a proposal, they would request a Proof from the user where their `action = proposal1`. For a single World ID, the output nullifier will always be the same number. More simply, a nullifier can be thought of as a one-time use ticket.

**Nullifiers in World ID ≤ 3.0**

```
   nullifier = H(identity_secret_key || rpId || action)
```
...
In this new version of the Protocol, we can no longer rely on the user's secret because there is no single secret. Further, if we were to rely on a secret that can be rotated, then the deterministic property of nullifiers would no longer hold. A single user would be able to perform an action multiple times.

**Introducing OPRF Nodes: Nullifiers in World ID 4.0**

To address this, we introduce a distributed multi-party network based on Verifiable Oblivious Pseudorandom Functions (or vOPRF for short — read more about OPRF in this [Systematization of Knowledge Paper](https://eprint.iacr.org/2022/302.pdf)). This network, called **OPRF Nodes**, provide the required secret entropy to ensure the determinism of nullifiers. In a nutshell, these functions are analogous to keyed-hash functions where a specific input is hashed with the inclusion of a secret.
...
The OPRF nodes then perform the computation on the blinded query  $Q'=k\cdot Q$ . Finally, the user can unblind the query using $\beta^{-1}$. This means deriving the output value of $H(x)\cdot k=n$ , but without any node ever learning the actual query, i.e. the nullifier is computed as $n=H'(x, (\beta^{-1}\cdot Q'))$.
...
The 4.0 Protocol introduces the concept of "sessions" for these use-cases, which are established between a user and an RP. A user can generate a Session Proof which proves they are still interacting with the same World ID with which the original session was established. When an RP requests a Session Proof, authenticator should display the request in a visually distinct manner, clearly signally the difference between anonymous proofs and pseudonymous ones.

A key requirement for Session Proofs is that the RP must already know which World ID they are querying before their request. By the nature of its construction, the proof will only valid if the same World ID is being queried.
...
![query proof.png](./query_proof.png)
...
| --- | --- | --- |
| **Deduplication**<br/>Being able to uniquely identify humans. | ✅ Available | Deduplication is accomplished through the issuance of PoH-specific credentials. |
| **Authentication**<br/>Being able to prove the person who initially enrolled is the person making a proof. | ⏳ Upcoming Protocol update | Upcoming in next update (World ID 4.1). |
| **Recovery**<br/>Being able to get back the identity and complete state in case of loss of all authentication mechanisms.  | ✅ Available | The Protocol now supports Recovery through delegated Recovery Agents. |
| **Revocation**<br/>An issuer is able to revoke a previously issued credential (e.g. if an attacker obtained a credential fraudulently) | ⏳ Upcoming Protocol update | Upcoming in a future update. |
| **Expiry**<br/>Efficacy of security mechanisms degrade over time, having a natural expiration addresses this. | ✅ Available | Credentials now support explicit expiration upon issuance. |
...
The product and technical specs for World ID 4.0 are publicly available. All the Protocol components are open source and permissively licensed. A testnet will be launching soon. To all contributors to the World ID Protocol, we strongly encourage anyone to review the proposed specs, try the Protocol out and provide comments on the proposed World ID 4.0 update.
