...
export const TOC = memo(function TOC(props: {toc: TOCType; contentRef?: MutableRefObject<HTMLElement | null>}) {
  ...
  const allIds = useMemo(
    () =>
      props.toc.reduce<Array<string>>(
        (result, item) => [...result, item.id, ...(item.children?.map((child) => child.id) || [])],
        [],
      ),
    [props.toc],
  )
    ...
    const ioCallback: IntersectionObserverCallback = (entries) => {
      entries.map((entry) => {
        if (entry.isIntersecting) {
          setActiveId((prevId) => (allIds.includes(entry.target.id) ? entry.target.id : prevId))
        }
      })
    }

    const io = new IntersectionObserver(ioCallback, {rootMargin: '-68px 0px -50% 0px'})
  ...
  useEffect(() => {
    ...
    const activeEl = navEl.querySelector(`[href="#${activeId}"]`) as HTMLElement | null
    const parentEl = activeEl?.parentElement

    if (activeEl && parentEl) {
      const trackerStart = Math.min(Math.max(activeEl.parentElement.offsetTop - 4, 0), navEl.clientHeight)
      const trackerHeight = Math.min(Math.max(activeEl.offsetHeight + 4, 0), navEl.clientHeight - trackerStart)
        ...
        {props.toc.map((parent, parentIdx) => (
          <Fragment key={parentIdx}>
            ...
            {parent.children?.map((child, childIdx) => (
              <Item {...child} key={childIdx} active={child.id === activeId} className="pl-4" />
            ))}
          </Fragment>
        ))}
      </ul>
