...
const linkClassName =
  "text-ffffff/40 hover:text-ffffff transition-colors w-[15px] h-[15px] md:w-5 md:h-5";
...
export const Article = memo(function Article(props: BlogPageProps) {
  ...
  const shareUrl = useMemo(
    () =>
      encodeURI(
        new URL(router.asPath, process.env.NEXT_PUBLIC_APP_URL).toString()
      ),
    [router.asPath]
  );
    ...
    <Fragment>
      <article className="mb-17 md:mb-64">
          ...
          <div className="col-start-2">
            <Breadcrumbs
              ...
              items={[
                ...
                { label: props.meta.title },
              ]}
            />

            <h1 className="font-bold text-28 md:text-42 mt-2.5 md:mt-5">
              {props.meta.title}
            </h1>
        ...
        >
          <aside>
            <div className="sticky top-32 space-y-13">
              <div className="grid gap-2 text-12">
                <span>
                  {dayjs().format("MMMM DD, YYYY")} (Updated{" "}
                  {dayjs().format("MMM DD YYYY")})
                </span>

                {props.meta.readTime && (
                  <span className="text-70868f capitalize">
                    {renderReadTime(props.meta.readTime)}
                  </span>
                )}
              </div>
          ...
          <main
            ref={contentRef}
            className="min-w-0 mt-12 md:mt-0 border-b border-afafaf md:border-transparent"
          >
